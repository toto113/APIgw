...
/**
 * A Spring {@link ContextLoader} that establishes a mock Servlet environment and {@link WebApplicationContext} so that Spring MVC stacks can be tested from within JUnit.
 */
public class MockWebApplicationContextLoader extends AbstractContextLoader {
	...
	public ApplicationContext loadContext(String... locations) throws Exception {
		...
		final MockServletConfig servletConfig = new MockServletConfig(servletContext, "radixtest");
		
		// Create a WebApplicationContext and initialize it with the xml and servlet configuration.
		final ConfigurableWebApplicationContext webApplicationContext = new XmlWebApplicationContext();
		servletContext.setAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE, webApplicationContext);
		webApplicationContext.setConfigLocations(locations);
		
		final DefaultServletHandlerConfigurer defaultServletHandlerConfigurer = new DefaultServletHandlerConfigurer(servletContext);
		defaultServletHandlerConfigurer.enable("radixtest");
