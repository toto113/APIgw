...
public class ClientPrePolicyEnforcerServiceImpl implements PolicyEnforcerService {
	...
	public void applyPolicy(ScopePolicies policies, byte[] clientID, String apiKey) throws PolicyException, JSONException {
		/*
		 * Applying policies of client before processing business concern.
		 */
		...
		if(packagePolicyTotalStatus == null) {
			packagePolicyTotalStatus = new HashMap<String, String>();
		}
		
		String currentPackageStatus = packagePolicyTotalStatus.get(packageStatusKey);
		
		// Checking policy of package
		String packageStatus = packagePolicyEnforcer.preCheck(packagePolicyProperties, currentPackageStatus);
		...
		for(Policy policy : policies.getPolicyList()) {
			...
			if("usageLimit".equals(policy.getPolicyTypeID())) {
				...
				serviceStatus = servicePolicyEnforcer.preCheck(servicePolicyProperties, currentServiceStatus);
				packagePolicyTotalStatus.put(serviceStatusKey, serviceStatus);
			}
		}
		
		policies.setStatus(packagePolicyTotalStatus);
	}
}
